...
v0.1.14 adds an in-advisor learned-preference diagnostics and reset surface on top of the runtime-verified v0.1.13 adaptive stack.

- Displays schema/persistence status and the current learned semantic-route count inside the existing scrollable advisor content.
- Lists each learned semantic route with score and `ACTIVE` / `QUIET` state.
- Shows score, negative-event count, full event counts, last event, event index, age, and soft-suppression state for the selected preference.
- Adds `Refresh` for the diagnostics view without changing topology.
- Adds `Reset Selected` to remove exactly one learned semantic preference through the existing backup-safe schema-1 persistence path.
- Adds a two-click `Reset All` / `Confirm Reset All` flow before clearing all learned preferences.
- Reset operations never emit connection-create/delete signals and never change topology.
- A newer unknown preference schema remains protected from destructive reset-all downgrade behavior.
- Existing candidate scoring, soft-suppression thresholds, guarded Accept, Undo, topology observation, and manual-choice observation are unchanged.
- No new polling timer and no new topology-mutation path were added.

## v0.1.14 runtime verification

The accepted v0.1.14 test ran on Godot 4.6.1 with the existing 12-mod compatibility stack across a diagnostics/reset session and a full restart.

Verified behavior:

- Session 1 loaded schema 1 with 4 learned routes at `event_index=15` before AAC reported `v0.1.14 ready`.
- The diagnostics UI attached successfully inside the existing scrollable advisor content.
- `Reset Selected` removed `heat_sink|4|heat|gpu_cluster|heat`, persisted the store from 4 entries to 3, and logged `topology_changed=false`.
- On the next scoring sample, the removed preference no longer influenced the route: GPU Cluster became the unique top Heat Sink connector-4 source at score 56, proving ranking refreshed from the modified preference store.
- The first `Reset All` click only armed confirmation and logged no topology mutation.
- The second click wrote an empty schema-1 store with `entries=0`, `event_index=0`, emitted the diagnostics reset event, and logged `topology_changed=false`.
- The next scoring sample returned the previously learned Heat Sink connector-4 contexts to ordinary unlearned ranking/tie behavior.
- After a full game restart, Session 2 loaded schema 1 with `entries=0`, `event_index=0` before AAC reported ready.
- Captured graph state after restart remained at 454 edges with 0 dangling, 0 non-reciprocal, and 0 resource-mismatch edges.
- No recurring diagnostics hitch was identified in the captured run.
...
## Repetitive-suggestion suppression

AAC reuses persistence schema 1 for reversible soft suppression.

A semantic route becomes soft-suppressed from the default suggestion slot only when its learned score is `<= -5` **and** it has at least two negative feedback events. Negative evidence includes `alternate`, `no_thanks`, and `undo`.

One `No thank you` or one quick Undo by itself is not enough to create persistent soft suppression. Soft-suppressed candidates remain legal, remain in the scored candidate set, and remain reachable through `Find different way`. If the player explicitly navigates to a soft-suppressed route, explicit player selection overrides the default suppression rule.
...
Schema-1 preferences are stored at:
...
4. Copy `guardipee14-AdaptiveAutoConnector-v0.1.14.zip` into `mods` without extracting it.
